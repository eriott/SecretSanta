import {
  EXCHANGE_CREATE_BEGIN,
  EXCHANGE_CREATE_FAILURE,
  EXCHANGE_CREATE_SUCCESS,
  REQUEST_EXCHANGES_FAILURE,
  REQUEST_EXCHANGES_SUCCESS
...
const initialState = {items: [], startExchange: {progress: false}};
...
export default function (state = initialState, action) {
  switch (action.type) {
    ...
    case REQUEST_EXCHANGES_SUCCESS:
      return Object.assign({}, state, {items: action.exchanges});
    case REQUEST_EXCHANGES_FAILURE:
      return Object.assign({}, state, {items: [], error: action.error});
    default:
      return state;
  }
}